...
export default function MotionEffects(){
 ...
 useEffect(()=>{
  ...
  const tiltSelector='.world-card,.capability-card,.channel-card,.project-direction';
  ...
  const pointer=(event:PointerEvent)=>{if(event.pointerType==='touch')return;const el=(event.target as HTMLElement).closest<HTMLElement>('[data-tilt]');if(!el)return;const rect=el.getBoundingClientRect();const x=(event.clientX-rect.left)/rect.width,y=(event.clientY-rect.top)/rect.height;el.style.setProperty('--tilt-x',`${(y-.5)*-8}deg`);el.style.setProperty('--tilt-y',`${(x-.5)*10}deg`);el.style.setProperty('--glow-x',`${x*100}%`);el.style.setProperty('--glow-y',`${y*100}%`);el.dataset.hovered='true'};
