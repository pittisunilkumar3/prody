...
export default function Experience({entertainment=false,backgroundOnly=false}:{entertainment?:boolean;backgroundOnly?:boolean}){
 ...
 useEffect(()=>{
  const video=film.current;if(!video)return;let raf=0,alive=true;
  const seek=()=>{if(!alive||!enabled.current||document.hidden||video.seeking||video.readyState<2||!Number.isFinite(video.duration))return;const target=Math.min(Math.max(0,video.duration-.045),wanted.current);if(Math.abs(video.currentTime-target)>1/48)video.currentTime=target};
  ...
  // Keep the newest requested position and apply it after an in-flight seek finishes.
  ...
  return()=>{alive=false;cancelAnimationFrame(raf);video.removeEventListener('loadeddata',loaded);video.removeEventListener('seeked',seek);video.removeEventListener('canplay',seek);removeEventListener('scroll',update);removeEventListener('resize',update);document.removeEventListener('visibilitychange',update)};
 },[sequence,motion]);
